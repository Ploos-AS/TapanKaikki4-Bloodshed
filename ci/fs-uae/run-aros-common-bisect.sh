...
OUT_DIR="${1:-build/fs-uae/aros-m3-common-bisect}"
SYSTEM_DIR="build/fs-uae/aros-system"
NATIVE_DIR="build-amiga"
mkdir -p "$OUT_DIR"

probes=(
  "common-a:tk4-common-a-probe:m3-common-a-main.txt"
  "common-b:tk4-common-b-probe:m3-common-b-main.txt"
  "common-c:tk4-common-c-probe:m3-common-c-main.txt"
  "common-d:tk4-common-d-probe:m3-common-d-main.txt"
...
run_probe() {
  ...
  [[ -f "$startup" ]] || { echo "ERROR: missing Startup-Sequence at $startup" >&2; exit 1; }
  cp "$NATIVE_DIR/$bin" "$root/$bin"
  mkdir -p "$root/save"
  cp "$startup" "$startup.original"

  cat > "$startup" <<EOF
SYS:C/Echo "COMMON_GUEST_STARTED=1" >SYS:common-started.txt
SYS:C/Stack 262144
SYS:C/Which $bin >SYS:common-which.txt
SYS:C/Echo "COMMON_BEFORE=1" >SYS:common-before.txt
SYS:$bin
SYS:C/Echo \$RC >SYS:common-rc.txt
SYS:C/Echo "COMMON_AFTER=1" >SYS:common-after.txt
EOF
  ...
  [[ -f "$root/common-after.txt" ]] && returned=yes
  {
    echo "PROBE=$name"
    echo "BINARY=$bin"
    echo "FS_UAE_EXIT=$rc"
    echo "GUEST_STARTED=$([[ -f "$root/common-started.txt" ]] && echo yes || echo no)"
    echo "BEFORE=$([[ -f "$root/common-before.txt" ]] && echo yes || echo no)"
    echo "MAIN=$main"
    echo "RETURNED=$returned"
    [[ -f "$root/common-rc.txt" ]] && tr -d '\r' < "$root/common-rc.txt" | sed 's/^/GUEST_RC=/'
    [[ -f "$root/common-which.txt" ]] && tr -d '\r' < "$root/common-which.txt" | sed 's/^/WHICH=/'
...

{
  ...
  echo "GATE=M3_COMMON_ARCHIVE_BISECT"
  for spec in "${probes[@]}"; do
    IFS=: read -r name bin marker <<<"$spec"
