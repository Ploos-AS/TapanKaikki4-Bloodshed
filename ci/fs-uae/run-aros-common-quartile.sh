...
probes=(
  "common-q1:tk4-common-q1-probe:m3-common-q1-main.txt"
  "common-q2:tk4-common-q2-probe:m3-common-q2-main.txt"
  "common-q3:tk4-common-q3-probe:m3-common-q3-main.txt"
  "common-q4:tk4-common-q4-probe:m3-common-q4-main.txt"
...
run_probe() {
  ...
  cat > "$startup" <<EOF
SYS:C/Echo "COMMON_QUARTILE_GUEST_STARTED=1" >SYS:common-quartile-started.txt
SYS:C/Stack 262144
SYS:C/Which $bin >SYS:common-quartile-which.txt
SYS:C/Echo "COMMON_QUARTILE_BEFORE=1" >SYS:common-quartile-before.txt
SYS:$bin
SYS:C/Echo \$RC >SYS:common-quartile-rc.txt
SYS:C/Echo "COMMON_QUARTILE_AFTER=1" >SYS:common-quartile-after.txt
EOF
  ...
  [[ -f "$root/common-quartile-after.txt" ]] && returned=yes
  {
    echo "PROBE=$name"
    echo "BINARY=$bin"
    echo "FS_UAE_EXIT=$rc"
    ...
    [[ -f "$root/common-quartile-rc.txt" ]] && tr -d '\r' < "$root/common-quartile-rc.txt" | sed 's/^/GUEST_RC=/'
    [[ -f "$root/common-quartile-which.txt" ]] && tr -d '\r' < "$root/common-quartile-which.txt" | sed 's/^/WHICH=/'
